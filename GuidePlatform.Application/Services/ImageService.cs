  ...
  public class ImageService : IImageService
  {
    ...
    private async Task<byte[]> ReadPhoto(ImageUploadDto dto)
    {
      try
      {
        ...
        {
          // Dosya yolu kontrolü - File path check
          if (File.Exists(dto.PhotoPath))
          {
            // Dosya türü kontrolü - File type check
            var extension = Path.GetExtension(dto.PhotoPath).ToLowerInvariant();
            var allowedExtensions = new[] { ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp" };

            if (!allowedExtensions.Contains(extension))
            {
              throw new ArgumentException($"Desteklenmeyen dosya türü: {extension}. Desteklenen türler: {string.Join(", ", allowedExtensions)}");
            }

            return await File.ReadAllBytesAsync(dto.PhotoPath);
          }
          else
          {
            // Dosya bulunamadı - File not found
            // Eğer bu bir Windows path ise (C:\ veya D:\) kullanıcıya uyarı ver
            if (dto.PhotoPath.StartsWith("C:\\") || dto.PhotoPath.StartsWith("D:\\") || dto.PhotoPath.StartsWith("E:\\"))
            {
              throw new FileNotFoundException($"Dosya bulunamadı: {dto.PhotoPath}. Bu bir yerel dosya yolu gibi görünüyor. Lütfen dosyayı Base64 formatında gönderin veya geçerli bir URL kullanın.");
            }
            else
            {
              throw new FileNotFoundException($"Dosya bulunamadı: {dto.PhotoPath}");
            }
          }
        }
